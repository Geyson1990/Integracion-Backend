...
using Contable.Authorization.Ldap;
using Contable.Authorization.Roles;
using Contable.Authorization.Users;
using Contable.Chat;
using Contable.Configuration;
using Contable.DashboardCustomization.Definitions;
using Contable.Debugging;
using Contable.DynamicEntityParameters;
...
using Contable.WebHooks;
using Contable.Authorization;

namespace Contable
{
    [DependsOn(
        typeof(ContableCoreSharedModule),
        typeof(AbpZeroCoreModule),
        typeof(AbpZeroLdapModule),
        typeof(AbpAutoMapperModule),
        typeof(AbpAspNetZeroCoreModule),
        typeof(AbpMailKitModule))]
    public class ContableCoreModule : AbpModule
    {
        public override void PreInitialize()
        {
            ...
            ContableLocalizationConfigurer.Configure(Configuration.Localization);
            ...
            Configuration.MultiTenancy.IsEnabled = ContableConsts.MultiTenancyEnabled;
            ...
            Configuration.ReplaceService(typeof(IEmailSenderConfiguration), () =>
            {
                Configuration.IocManager.IocContainer.Register(
                    Component.For<IEmailSenderConfiguration, ISmtpEmailSenderConfiguration>()
                             .ImplementedBy<ContableSmtpEmailSenderConfiguration>()
                             .LifestyleTransient()
                );
            });
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(ContableCoreModule).GetAssembly());
        }
        ...
        {
            Configuration.Caching.Configure(LoginCacheItem.CacheName, cache =>
            {
                cache.DefaultSlidingExpireTime = LoginCacheItem.DefaultSlidingExpireTime;
            });
        }
    }
}