...
using Contable.Auditing;
using Contable.Configuration;
using Contable.EntityFrameworkCore;
using Contable.MultiTenancy;
using Contable.Web.Startup.ExternalLoginInfoProviders;
using Contable.Worker;

namespace Contable.Web.Startup
{
    [DependsOn(
        typeof(ContableWebCoreModule)
    )]
    public class ContableWebHostModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public ContableWebHostModule(
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(ContableWebHostModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            if (bool.Parse(_appConfiguration["App:AllowBackgroundService"]))
            {
