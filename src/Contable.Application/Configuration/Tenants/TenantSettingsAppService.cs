    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        public async Task<TenantSettingsEditDto> GetAllSettings()
        {
            var settings = new TenantSettingsEditDto
            {
                ...
                Extras = await GetExtraSettingsAsync()
            };
        ...
        private async Task<TenantExtraSettings> GetExtraSettingsAsync()
        {
            return new TenantExtraSettings
            {
                SocialConflictAlertTemplate = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.SocialConflictAlertTemplate, AbpSession.GetTenantId()),
                TaskManagementAlertSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.TaskManagementAlertSubject, AbpSession.GetTenantId()),
                TaskManagementAlertNotificationSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.TaskManagementAlertNotificationSubject, AbpSession.GetTenantId()),
                TaskManagementAlertExpirationSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.TaskManagementAlertExpirationSubject, AbpSession.GetTenantId()),
                TaskManagementAlertTemplate = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.TaskManagementAlertTemplate, AbpSession.GetTenantId()),

                SocialConflictTaskManagementAlertSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.SocialConflictTaskManagementAlertSubject, AbpSession.GetTenantId()),
                SocialConflictTaskManagementAlertNotificationSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.SocialConflictTaskManagementAlertNotificationSubject, AbpSession.GetTenantId()),
                SocialConflictTaskManagementAlertExpirationSubject = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.SocialConflictTaskManagementAlertExpirationSubject, AbpSession.GetTenantId()),
                SocialConflictTaskManagementAlertTemplate = await SettingManager.GetSettingValueForTenantAsync(AppSettings.Template.SocialConflictTaskManagementAlertTemplate, AbpSession.GetTenantId())
            };
        }
        ...
        public async Task UpdateAllSettings(TenantSettingsEditDto input)
        {
            ...
            await UpdateExtraSettingsAsync(input.Extras);
        ...
        private async Task UpdateExtraSettingsAsync(TenantExtraSettings input)
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.SocialConflictAlertTemplate, input.SocialConflictAlertTemplate);

            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.TaskManagementAlertSubject, input.TaskManagementAlertSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.TaskManagementAlertNotificationSubject, input.TaskManagementAlertNotificationSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.TaskManagementAlertExpirationSubject, input.TaskManagementAlertExpirationSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.TaskManagementAlertTemplate, input.TaskManagementAlertTemplate);

            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.SocialConflictTaskManagementAlertSubject, input.SocialConflictTaskManagementAlertSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.SocialConflictTaskManagementAlertNotificationSubject, input.SocialConflictTaskManagementAlertNotificationSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.SocialConflictTaskManagementAlertExpirationSubject, input.SocialConflictTaskManagementAlertExpirationSubject);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Template.SocialConflictTaskManagementAlertTemplate, input.SocialConflictTaskManagementAlertTemplate);
        }
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !ContableConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
